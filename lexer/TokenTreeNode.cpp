...
namespace racc::lexer {
    ...
    sourcemap::Location TokenTreeNode::getPosition(const sourcemap::SourceMap &sources) const {
    ...
    std::string TokenTreeNode::toString(const sourcemap::SourceMap &sources, uint32_t indent) const {
        if (isTokenTree()) {
