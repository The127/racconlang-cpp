...
namespace racc::lexer {
    ...
    sourcemap::Location TokenResult::getLocation(const sourcemap::SourceMap &sources) const {
        return sources.getLocation(getStart());
    }

    std::string TokenResult::toString(const sourcemap::SourceMap &sources) const {
        if (isError()) {
